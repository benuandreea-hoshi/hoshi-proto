...
function Building(){
  ...
  return (
    <div className="grid gap-4 md:gap-6">
      <Section title="Total Building Services Expenditures" right={
        <div className="flex items-center gap-2">
          <span className="text-slate-300 text-xs">Year</span>
          <select className="px-2 py-1 rounded-lg text-sm" style={{background:"var(--panel-2)",border:"1px solid var(--stroke)",color:"var(--text)"}} value={year} onChange={e=>setYear(e.target.value)}>
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-slate-300 mb-3">
          <span><i className="legend-dot" style={{background:"#34d399"}}/> Actual</span>
          <span><i className="legend-dot" style={{background:"#dde3ea22",border:"1px solid #233147"}}/> Budget</span>
          <span><i className="legend-dot" style={{background:"#ef4444"}}/> Overrun</span>
        </div>

        {/* Bars */}
        <div className="space-y-8">
          {rows.map(([k,A,B],i)=>{
            const bw=B/M.max*100, aw=Math.min(A,B)/M.max*100, ow=A>B?(A-B)/M.max*100:0;
            const pct=B?(((A-B)/B)*100).toFixed(0)+"%":"–";
            return (
              <div key={i} className="hb">
                <div className="hb-l"><div className="text-slate-100 font-medium leading-tight">{k}</div></div>
                <div className="hb-w" title={`${k} • ${fmt(A)} vs ${fmt(B)} (${pct})`}>
                  <div className="hb-t" style={{width:`${bw}%`}}/>
        ...
        <div className="mt-5 grid md:grid-cols-3 gap-3 text-sm">
          <div className="rounded-xl p-3" style={{background:"var(--panel-2)",border:"1px solid var(--stroke)"}}>
            ...
            <div className="text-slate-100 mt-1">Red segments show spend **above** budget — prioritise these lines.</div>
          </div>
          <div className="rounded-xl p-3" style={{background:"var(--panel-2)",border:"1px solid var(--stroke)"}}>
